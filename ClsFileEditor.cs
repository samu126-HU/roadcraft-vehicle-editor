    ...
    public class ClsFileEditor : UserControl
    {
        ...
        private void InitializeComponent()
        {
            ...
            _clsFilesTabControl = new TabControl
            {
                ...
                SizeMode = TabSizeMode.Normal, // Changed from Fixed to Normal for dynamic sizing
        ...
        private void OnNodeAfterLabelEdit(object? sender, NodeLabelEditEventArgs e, string fileName)
        {
            ...
            try
            {
                ...
                if (colonIndex > 0 && colonIndex < newLabel.Length - 1)
                {
                    ...
                    if (UIUtils.IsNumericType(oldValue?.GetType())) // FIXED: Use UIUtils directly
                    {
                        newValue = UIUtils.TryParseNumericValue(valuePart); // FIXED: Use UIUtils directly
                        
                        if (newValue == null && !string.IsNullOrEmpty(valuePart))
                        {
                            // Show error message for invalid input
                            MessageBox.Show($"Invalid numeric value: '{valuePart}'\n\nPlease enter a valid number (integer or decimal).",
                                          GlobalConstants.Messages.InvalidInput, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                        
                        // If empty text for numeric field, set to 0 based on original type
                        if (newValue == null && string.IsNullOrEmpty(valuePart))
                        {
        ...
        private void EditNodeValue(TreeView treeView, string fileName, TreeNode? selectedNode = null)
        {
            ...
            var labelPath = new Label
            {
                ...
                Font = GlobalConstants.Fonts.DefaultBoldFont // FIXED: Use cached font
            };
            ...
            var isNumeric = UIUtils.IsNumericType(nodeData.Value?.GetType()); // FIXED: Use UIUtils directly
            TextBox textBoxValue;
            
            if (isNumeric)
            {
                // Create numeric TextBox with validation
                textBoxValue = UIUtils.CreateNumericTextBox( // FIXED: Use UIUtils directly
                    new Point(100, 68),
                    new Size(320, 20),
                    nodeData.Value);
                
                // Override color settings to use GlobalConstants
                textBoxValue.ForeColor = GlobalConstants.Colors.TextPrimary;
                
                // Add help text for numeric input
                var helpLabel = new Label
                {
                    Text = "💡 This field accepts integers (42) or decimals (42.5). Type conversion between int/double is automatic.",
                    Location = new Point(10, 95),
                    Size = new Size(410, 30),
                    Font = GlobalConstants.Fonts.SmallFont, // FIXED: Use cached font
                    ForeColor = Color.FromArgb(100, 100, 100)
                };
                form.Controls.Add(helpLabel);
                
                // Adjust form size for help text
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    if (isNumeric)
                    {
                        ...
                        newValue = UIUtils.TryParseNumericValue(textBoxValue.Text); // FIXED: Use UIUtils directly
        ...
        private object? ConvertStringToType(string value, Type? targetType)
        {
            ...
            try
            {
                ...
                else if (UIUtils.IsNumericType(targetType)) // FIXED: Use UIUtils directly
                {
                    // Use smart numeric parsing for all numeric types
                    var numericValue = UIUtils.TryParseNumericValue(value); // FIXED: Use UIUtils directly
        ...
        private void OnClsPropertyChanged(string fileName, string path, object? newValue, object? oldValue)
        {
            try
            {
                ...
                {
                    // Handle null values by trying to set the value anyway
                    // Some parsers might handle null differently
                    try
                    {
                        updateSuccessful = clsParser.SetValue(path, newValue);
                    }
                    ...
                    {
                        // If setting null fails, try setting an empty string for string types
        ...
        private void RefreshTreeViewForFile(string fileName, Dictionary<string, object> updatedData)
        {
            ...
            foreach (TabPage tabPage in _clsFilesTabControl.TabPages)
            {
                    ...
                    tabPage.Controls.Count > 0 && tabPage.Controls[0] is TreeView treeView)
                {
                    ...
                    // Note: Selected node restoration could be added here if needed
                    // For now, we'll just preserve the expanded state
